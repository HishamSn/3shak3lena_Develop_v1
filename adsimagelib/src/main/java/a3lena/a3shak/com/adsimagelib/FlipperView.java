package a3lena.a3shak.com.adsimagelib;
...
public class FlipperView {
    ...
    public FlipperView setLogo(String imageUrl) {
        if (logo != 0) {
            throw new IllegalStateException("Can't set multiple images");
        }
...
//        ImageView description = (ImageView) v.findViewById(R.id.logo);
//        description.getBackground().setAlpha(10);
//        autoSliderImage.setImageAlpha(120);
    ...
    private void bindData(View v, ImageView autoSliderImage) {
        ...
        try {
            ...
            if (imageUrl != null) {
                Picasso.with(context).load(Uri.parse(imageUrl)).fit()
                        .into(autoSliderImage);

            } else {
                Picasso.with(context).load(imageRes).fit().into(autoSliderImage);
