...
//    private CircleIndicator pagerIndicator;
    /*
        * Scroll Time in seconds
         */
    private int scrollTimeInSec = 60;
    /*
...
//        startAutoCycle();
    ...
    * @params context for Inflater
    ...
    private void setLayout(Context context) {
        ...
        lolo = (ImageView) view.findViewById(R.id.lolo);
...
//        pagerIndicator.setViewPager(mFlippingPager);
    ...
    public void setAlpha(){
        mFlippingPager.setAlpha((float) 0.5);
    }

    public void setImage(Context ctx, String url, int type){
        if(type == 0){
            lolo.setVisibility(GONE);
        }else{
            Picasso.with(ctx).load(Uri.parse(url))
                    .into(lolo);
        }
    }
